"""
Frequency-domain (spectral) disruption for mid-band pixel watermark carriers.

Implements a research-aligned *local* transform using only NumPy FFTs:

  1. Convert to YCbCr; emphasize luma.
  2. Adaptive carrier detection: mid-band magnitude peaks that look like
     fixed-resolution periodic marks (local z-score + conjugate pair).
  3. Multi-pass dampening of those bins + mid-band phase jitter.
  4. Inverse FFT, blend back for PSNR control.

No cloud. No fingerprint DB download. Carriers are estimated per-image.

Best-effort only — not a cryptographic guarantee that any detector will fail.
"""
...
def spectral_attack_image(
    ...
    higher blend-back (favor visual fidelity).
    """
    p = _params(strength)
    if fidelity_first:
