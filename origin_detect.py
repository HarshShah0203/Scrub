"""
Origin / provenance detector for AI-generated media.

Goal: given a file, decide whether it is likely to carry a *robust* invisible
watermark (today: Google SynthID family for images / video / audio), so the
cleaning pipeline can auto-escalate its attack strength.

Signals we inspect (offline, no network):

    * Image EXIF tags: `Software`, `Make`, `Model`, `HostComputer`,
      `ProcessingSoftware`, `ImageUniqueID`.
    * XMP and XML-ish metadata embedded in the file (scanned as bytes):
      `xmp:CreatorTool`, `dc:creator`, `xmpMM:Ingredients`, C2PA
      `claim_generator`, `jumbf` container boxes.
    * PNG `tEXt` / `iTXt` chunks.
    * ffprobe format + stream tags for video / audio: `encoder`, `handler_name`,
      `comment`, `com.apple.quicktime.*`, `creation_time`, and free-text tags.

We never trust any *single* signal; we combine them and return a structured
result with the list of reasons so the UI can explain itself.

Strings searched (case-insensitive substrings):

    * Google Imagen / Gemini / Nano Banana / DeepMind / SynthID / Lyria
    * Meta AudioSeal
    * OpenAI Sora / DALL-E claim_generator strings
    * Common C2PA claim_generator issuers

Reliability note: if the upstream creator stripped metadata before handing
the file to us, detection will miss. In that case we fall back to the
Near-lossless default — which still hits the C2PA-less and SynthID-less
watermarks cleanly, and gently hits SynthID-for-images.
"""
