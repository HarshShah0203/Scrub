"""
Signal-disruption pipelines for images, video, and audio.

Applies *signal-processing transforms* drawn from public watermark-robustness
literature. Individual steps may degrade certain invisible watermark-like
signal families discussed in research and community analysis:

    - mid-band pixel carriers: sub-LSB noise + mild smoothing
    - frequency / latent-style marks: small resize cycles, crop/pad, JPEG
    - registration-sensitive marks: asymmetric crop+pad
    - audio spectral-mask style signals: resample + filters + codec swap

Best-effort only. Strength settings trade fidelity for stronger disruption.
"Medium" is a sensible default for many files.

An optional diffusion regeneration path (`diffusion_regen_image`) activates
only if `diffusers` + `torch` are installed; they are not hard dependencies.

See NOTICE.md for intended use. Use only on media you have rights to modify.
"""
...
def _image_params(strength: Strength) -> dict:
    if strength == "near_lossless":
        ...
        #   * 1-pixel sub-pixel jitter (perturbs registration-sensitive
        #     carriers)
...
def _video_params(strength: Strength) -> dict:
    if strength == "near_lossless":
        # For video the codec swap is already the largest disruption step
...
def attack_image(
    ...
    """
    Run the image signal-disruption chain and save an output with metadata
    dropped (Pillow re-save discards EXIF/info by default here).

    Order:
      1. optional visible corner-badge inpaint
      2. optional mid-band FFT spectral disruption
    ...
    a JPEG buffer at the preset quality; this re-quantizes mid-freq DCT bins.
    """
    ...
    if fmt == "JPEG":
        ...
        if force_jpeg_roundtrip:
            # Explicit JPEG round-trip in-memory first (re-quantizes DCT bins that often carry marks
...
def attack_video(
    ...
    """
    Run the video signal-disruption chain via ffmpeg.
    Always re-encodes (stream-copy would keep original pixels).
    Metadata is dropped via -map_metadata -1.

    If use_spectral=True and the clip is short enough, each frame also gets
    adaptive FFT carrier disruption. Long clips use the ffmpeg chain only.
    """
    spectral_note = ""
